...
def make_variational_layer(
    ...
    z_log_var_dense_linear = Dense(size, kernel_initializer="glorot_uniform")(
        input_layer
    )
    ...
    z = Lambda(sampling_function, output_shape=(size,))(
...
def stacked_vae(
    ...
    if hidden_dims:
        for hidden_dim in reversed(hidden_dims):
            h = Dense(
                hidden_dim, kernel_initializer="glorot_uniform", activation="relu"
            )(prev)
            prev = h
    else:
        h = Dense(latent_dim, kernel_initializer="glorot_uniform", activation="relu")(
            prev
        )
